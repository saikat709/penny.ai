...
const ChatHistorySidebar: React.FC<ChatHistorySidebarProps> = ({ chats, selectedChatId, onSelectChat }) => {
  return (
    <aside className="w-64 bg-gradient-to-b from-gray-900 to-gray-800 border-r border-gray-700 text-white flex flex-col p-4">
      <h3 className="mb-2 text-lg font-semibold text-pink-300 tracking-wide">Chat History</h3>
      <div className="w-full h-[1px] bg-red-500 mb-1"> </div>
      <ul className="list-none p-0">
        {chats.map(chat => (
          <li
            key={chat.id}
            className={`mb-0 px-4 py-2 rounded-lg cursor-pointer transition-colors font-medium ${
